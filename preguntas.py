...
def pregunta_02():
    ...
    # Lea el archivo `gm_2008_region.csv` y asignelo al DataFrame `df`
    df = pd.read_csv("C:\\Users\\jvalencia\\OneDrive - Elico Group S.A.S\\Documentos\\GitHub\\regresion-lineal-basica-gapminder-jdvalen0UNAL\\gm_2008_region.csv")

    # Imprima las dimensiones del DataFrame
    print(df.shape)

    # Imprima la correlación entre las columnas `life` y `fertility` con 4 decimales.
    correlation_life_fertility = round(df[['life', 'fertility']].corr().loc['life', 'fertility'], 4)
    print(correlation_life_fertility)

    # Imprima la media de la columna `life` con 4 decimales.
    mean_life = round(df['life'].mean(), 4)
    print(mean_life)
    ...
    correlation_GDP_life = round(df[['GDP', 'life']].corr().loc['GDP', 'life'], 4)
    print(correlation_GDP_life)
